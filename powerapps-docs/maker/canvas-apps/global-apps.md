...
앱을 빌드하고 실행하는 동안 PowerApps에서 표시되는 텍스트는 다양한 언어로 번역되었습니다.  따라서 메뉴 항목, 대화 상자, 리본 탭은 물론, 기타 텍스트를 모국어로 보게 될 것입니다.  날짜와 숫자의 입력과 표시도 특정 언어와 지역에 맞게 수정됩니다.  예를 들어, 전 세계의 일부 지역은 **합니다.** (점 또는 마침표) 소수 구분 기호로 사용 하 고 다른 한 **,** (쉼표).  
...
* 신호 레코드: **Compass.Heading**, **위치 합니다. Latitude**, **App.ActiveScreen**, ...
...
강조 표시를 두 버전 간에 변경 하는 연산자를 보여 줍니다.  속성 선택 연산자 **.** (점 또는 마침표)에서 **Slider1.Value** 는 항상 소수 구분 기호는 상관 없이 동일 합니다.
...
* **[값](functions/function-value.md)**: 숫자 값을 텍스트 문자열에서 숫자로 변환합니다.
...
* *문자열, 필수*: 사용자는 문자열입니다. 예를 들어 문자열을 **텍스트 입력** 컨트롤에 입력하고 **Text** 속성으로 컨트롤에서 읽습니다.
* *언어에서 선택적*: 언어를 해석 하는 *문자열*합니다.  기본적으로 사용자의 언어 설정입니다.
