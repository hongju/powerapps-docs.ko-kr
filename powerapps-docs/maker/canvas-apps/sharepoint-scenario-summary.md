...
3. 앱의 찾아보기 갤러리에서 첫 번째 항목의 ![항목 아이콘을 추가하여](./media/sharepoint-scenario-summary/icon-add-item.png) 클릭하거나 탭하여 새 항목을 만듭니다.
4. 다음 값으로 양식을 작성합니다.
...
5. 확인 표시 아이콘을 ![클릭하거나 누른](./media/sharepoint-scenario-summary/icon-check-mark.png) 클릭하거나 탭한 다음 브라우저 탭을 닫습니다.
...
## <a name="step-2-approve-the-project"></a>2 단계: 프로젝트 승인
...
5. 종 모양 아이콘을 ![클릭하거나 누른](./media/sharepoint-scenario-summary/icon-check-mark.png) 클릭하거나 탭하여  변경 내용을 SharePoint 목록에 적용합니다.
...
5. 프로젝트가 승인에서 프로젝션된 시작 날짜로 이동하는 데 걸리는 시간을 보여주는 표를 검토합니다.
    ...
    이 시각화를 만들 때 설명한 대로 Irvin Sayers가 할당한 프로젝트는 나머지보다 훨씬 오래 걸리는 두 개의 프로젝트를 포함하기 때문에 시작하는데 오래 걸립니다.
...
4. 경고를 만든 사용자의 받은 편지함을 확인합니다(이 경우에 Megan Bowen임).
   
    ![Power BI에서 보낸 경고 전자 메일](./media/sharepoint-scenario-summary/09-06-04-email-powerbi.png)
5. 데이터 경고 흐름에 추가한 사용자의 받은 편지함을 확인합니다(이 경우에 Allan DeYoung임).
