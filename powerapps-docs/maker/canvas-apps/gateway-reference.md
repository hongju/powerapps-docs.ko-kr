...
게이트웨이는 Azure Service Bus에 대한 아웃바운드 연결을 만듭니다. 아웃 바운드 포트에서 통신합니다. TCP 443 (기본값), 5671, 5672, 9350 ~ 9354 합니다. 게이트웨이에는 인바운드 포트가 필요하지 않습니다.
...
**질문:** SQL Azure 같은 클라우드에서 데이터 원본에 대 한 게이트웨이가 필요 합니까?  
**대답:** 아니요. 게이트웨이는 온-프레미스 데이터 원본에만 연결됩니다.

**질문:** 실제 Windows 서비스는 호출 하는?  
**대답:** 게이트웨이 서비스에서 라고 **Power BI 엔터프라이즈 게이트웨이 서비스**합니다.

**질문:** 사항이 모든 인바운드 연결 게이트웨이를 클라우드에서?  
**대답:** 아니요. 게이트웨이는 Azure Service Bus로 아웃바운드 연결을 사용합니다.

**질문:** 아웃 바운드 연결을 차단 하면 어떻게 됩니까? 어떻게 해야 하나요?  
**대답:** 위에서 게이트웨이에 사용 하는 포트 및 호스트 목록을 참조 하세요.

**질문:** 게이트웨이는 데이터 소스와 동일한 컴퓨터에 설치 해야 합니까?  
**대답:** 아니요. 게이트웨이는 제공된 연결 정보를 사용하여 데이터 원본에 연결합니다. 이 의미에서 게이트웨이를 클라이언트 애플리케이션으로 생각합니다. 제공된 서버 이름에만 연결할 수 있어야 합니다.

**질문:** 게이트웨이에서 데이터 원본에 쿼리를 실행 하기 위한 대기 시간 이란? 최상의 아키텍처는 무엇인가요?  
**대답:**  네트워크 대기 시간을 줄이기 위해 가능한 데이터 원본에 가깝게 게이트웨이 설치 합니다. 실제 데이터 원본에 게이트웨이를 설치할 수 있는 경우 발생하는 대기 시간이 최소화됩니다. 데이터 센터도 고려해야 합니다. 예를 들어 서비스가 미국 서부 데이터 센터를 사용하고 Azure VM에서 호스팅되는 SQL Server가 있다면 미국 서부에도 Azure VM이 있어야 합니다. 이렇게 하면 대기 시간이 최소화되고 Azure VM에서 송신 요금을 피할 수 있습니다.

**질문:** 네트워크 대역폭 요구 사항이 있나요?  
**대답:** 네트워크 연결에 대 한 양호한 처리량을 갖는 것이 좋습니다. 각 환경이 다르며 전송되는 데이터 양은 결과에 영향을 줍니다. ExpressRoute를 사용하면 온-프레미스와 Azure 데이터 센터 간에 처리량 수준을 보장하는 데 도움이 될 수 있습니다.
...
**질문:** 클라우드로 다시 결과 전송 하는 방법  
**대답:** Azure Service Bus를 통해 수행 됩니다. 자세한 내용은 [작동 방법](gateway-reference.md#how-the-gateway-works)을 참조하세요.

**질문:** 내 자격 증명을 저장 하는 위치  
**대답:** 데이터 원본에 입력 한 자격 증명은 게이트웨이 클라우드 서비스에 암호화 되어 저장 됩니다. 자격 증명은 게이트웨이 온-프레미스에서 암호 해독됩니다.

**질문:** 경계 네트워크 (DMZ, 완충 지역 및 스크린 된 서브넷이 라고도 함)에서 게이트웨이 배치할 수 있습니까?  
**대답:** 게이트웨이 데이터 원본에 대 한 연결을 필요합니다. 데이터 원본이 경계 네트워크에 없는 경우 게이트웨이가 연결되지 못할 수 있습니다. 예를 들어 SQL Server를 실행하는 컴퓨터가 경계 네트워크에 있지 않을 수 있으며 경계 네트워크에서 해당 컴퓨터에 연결할 수 없습니다. 게이트웨이를 경계 네트워크에 배치한 경우 게이트웨이는 SQL Server를 실행 중인 컴퓨터에 연결할 수 없습니다.

#### <a name="high-availabilitydisaster-recovery"></a>고가용성/재해 복구
**질문:** 게이트웨이와 함께 고가용성 시나리오를 사용 하기 위한 모든 계획이 있습니까?  
**대답:** 동일한 클러스터에 2 개 이상의 게이트웨이 조인 하 여 고가용성이 사용 됩니다.  고가용성 게이트웨이 클러스터를 사용하려면 온-프레미스 데이터 게이트웨이 2017년 11월 업데이트 이상이 필요합니다.  자세한 내용은 [블로그 게시물 알림](https://powerapps.microsoft.com/en-us/blog/gateway-high-availability-for-powerapps-and-flow)을 참조하세요.

**질문:** 어떤 옵션을 재해 복구에 사용할 수 있습니까?  
**대답:** 복구 키를 복원 하거나 이동할 게이트웨이를 사용할 수 있습니다. 게이트웨이 설치하는 경우 복구 키를 지정합니다.

**질문:** 복구 키의 이점은 무엇 인가요?  
**대답:** 마이그레이션 또는 재해 발생 후 게이트웨이 복구 하는 방법을 제공 합니다.

#### <a name="troubleshooting"></a>문제 해결
**질문:** 게이트웨이 로그는 어디에 있습니까?  
**대답:** 참조 [도구](gateway-reference.md#tools) 이 항목의에서 뒷부분에 있습니다.

**질문:** 중인 쿼리가 무엇 인지 볼 수는 어떻게 온-프레미스 데이터 원본에 전송?  
**대답:** 전송 중인 쿼리를 포함 하는 쿼리 추적을 사용할 수 있습니다. 문제 해결이 완료되면 원래 값으로 다시 변경해야 합니다. 쿼리 추적을 사용된 상태로 두면 로그가 더 커질 수 있습니다.
...
#### <a name="error-failed-to-add-user-to-group---2147463168---pbiegwservice---performance-log-users---"></a>오류: 그룹에 사용자를 추가 하지 못했습니다.  (-2147463168   PBIEgwService   Performance Log Users   )
