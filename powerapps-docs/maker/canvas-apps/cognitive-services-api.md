...
Azure Cognitive Services는 Api, Sdk 및 응용 프로그램을 더 지능적이고 유용하게 사용할 수 있는 검색 가능한 서비스의 집합입니다. 이 서비스를 통해 사용자는 지능형 기능(예: 감정 및 비디오 감지, 안면, 음성 및 시각 인식, 음성 및 언어 이해)을 애플리케이션에 쉽게 추가할 수 있습니다.
...
2. [이 페이지](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextAnalytics),다음 이미지와 같이 Text Analytics API에 대한 정보를 입력 합니다. **F0**(무료) 가격 책정 계층을 선택합니다.
...
1. [powerapps.com](https://web.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc)에서 **빈 페이지의 캐버스 앱** > **앱 이름**을 입력하고 > ![Phone 앱 아이콘](./media/cognitive-services-api/icon-phone-app.png)(전화) > **만들기**를 선택합니다.
...
8. **세로(비어 있음)** 갤러리 컨트롤을 추가한 다음 **레이블** 컨트롤을 갤러리에 추가합니다. 이 갤러리에는 핵심 문구 API 호출의 결과가 저장됩니다. 앱은 이제 다음 이미지와 같습니다.
...
* 각 호출 내에서 적합한 매개 변수를 지정합니다.
  ...
  * **keyphrases()**과 **detectsentiment()** 에서 **language**는 "en"으로 하드 코딩되지만, 앱의 일부 논리에 따라 이 매개 변수를 전달할 수 있습니다. 예를 들어 언어를 먼저 감지한 다음, **DetectLanguage()** 에서 반환하는 내용에 따라 이 매개 변수를 설정할 수 있습니다.
...
이 문서의 시작 부분에서 Text Analytics API 페이지와 이 앱의 출력을 비교할 경우 동일한 결과를  수 있습니다.
