...
SharePoint 사이트에서 목록을 만든 다음, 해당 목록에 이러한 열을 추가 합니다.
...
1. 명령 모음에서 **PowerApps**를 선택한 후 **양식 사용자 지정**을 선택 합니다.
...
1. 필드 목록의 맨 아래로 **가용성** 필드를 끌기 합니다.

    지정 된 순서로 필드가 나타납니다.

1. **첨부 파일** 필드를 마우스로 가리키고, 나타나는 줄임표 (...)를 선택한 다음, **제거**를 선택 합니다.
...
**세부 정보**가 예로 설정되는 경우에만 **가격**, **가용성** 및 **색** 필드가 나타나도록 구성할 수 있습니다.

1. 왼쪽된 탐색 모음에서 **Details_DataCard1**을 확장하여, **DataCardValue**의 끝에 표시 되는 숫자를 기록해 둡니다.

1. **색**, **가용성**, 및 **가격** 카드의 **Visible** 속성을 다음 수식으로 설정합니다. (필요시 이전 단계에서 적어둔 숫자로 대체합니다):
...
## <a name="further-customize-your-form"></a>추가로 양식 사용자 지정

1. 목록을 열어, 명령 모음에서 **새로 만들기**를 선택한 후, 폼의 맨 위 근처 **사용자 지정**을 선택 합니다.

1. 이러한 항목을 설명 하는 것과 같은 다양한 방법으로 양식을 사용자 지정 합니다.
...
1. SharePoint 목록에서 오른쪽 위 모서리 근처의 기어 아이콘을 선택하여 설정 페이지를 연 다음, **목록 설정**을 선택합니다.
...
**Q:** SharePoint 목록에서 데이터를 관리 하는 양식을 언제 사용자 지정해야 하고 언제 독립 실행형 앱을 만들어야 하나요?
...
**Q:** 동일한 기능을 사용 하는 목록을 사용자 지정하고 앱을 만들 수 있나요?

**A:** 예.

**Q:** 조직에서 기본 환경과 다른 환경에서 양식을 사용자 지정할 수 있나요?

**A:** 아니요.

### <a name="manage-your-custom-form"></a>사용자 지정 양식 관리

**Q:** 어떻게 다른 사용자와 양식을 쉽게 공유할 수 있나요?

**A:** 폼을 열고 **링크 복사**를 선택한 다음 폼을 사용 하려는 모든 사용자에게 링크를 보냅니다.
...
1. 다른 브라우저 탭의 **버전** 페이지로 돌아가서 복원할 버전을 찾은 다음, **복원**을 선택합니다.
    ...
    > [!NOTE]
    > 폼이 다른 사용자에 의해 잠겨 있기 때문에 복원에 실패 한다는 오류 메시지를 받게 되면, 폼을 잠금 해제할 때까지 기다린 후 다시 시도 하십시오.

**Q:** 다른 목록으로 내 양식을 이동할 수 있습니까?

**A:** 아니요.

### <a name="administer-your-custom-form"></a>사용자 지정 양식 관리

**Q:** 양식을 공유 하려면 어떻게 해야 하나요?
...
**A:** SharePoint 권한을 가진 모든 사용자가 디자인, 또는 연결 된 목록을 편집, 관리 합니다.

**Q:** 목록 양식을 만들기 또는 사용자 지정 하는 데 PowerApps 라이선스가 필요 합니까?

**A:** [PowerApps가 포함 된 Office 365 계획](https://docs.microsoft.com/power-platform/admin/pricing-billing-skus.md#licenses)이 필요 합니다.

**Q:** 게스트 사용자들이 사용자 지정 양식이 있는 목록에 액세스 하는 경우 어떻게 되나요?

**A:** 게스트 사용자는 PowerApps를 사용 하여 사용자 지정된 목록 양식에 액세스 하려고 하면 오류 메시지를 가져옵니다.

**Q:** 관리자로서, 조직의 모든 사용자 지정된 양식 목록을 어떻게 가져오나요?

**A:** PowerApps의 테넌트 관리자이거나 조직의 기본 PowerApps 환경에 대한 환경 관리자 권한을 가지고 있는 경우, 다음을 수행 합니다.

1. [PowerApps 관리 센터](https://admin.powerapps.com)의 환경 목록에서 조직의 기본 환경을 선택합니다.

1. 기본 환경 페이지의 맨 위에서 **리소스**를 선택합니다.

1. 앱 목록에서, **SharePoint Form** 앱 유형을 찾으세요 -그것이 사용자 지정된 양식입니다.
