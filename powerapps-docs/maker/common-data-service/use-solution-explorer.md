...
> [!NOTE]
>  솔루션 환경은 온라인 환경 및 환경 버전 9.1.0.267 이상에서만 사용할 수 있습니다. 버전을 확인하려면 [PowerApps 관리 센터](https://admin.powerapps.com/) 환경> 환경 선택 > 세부 정보 탭을 선택합니다. 이전 버전의 경우 솔루션을 선택하면 기본 환경에서 열립니다. 
...
- 기존 SDK 어셈블리를 추가한 후에는 솔루션에 표시되지 않습니다. 
- 캔버스 앱이 관리형 솔루션으로 압축된 경우에도 새 환경에서는 관리자가 이를 편집할 수 있습니다.
- 캔버스 앱에는 종속성을 사용할 수 없습니다.
- 관리형 솔루션을 삭제해도 다른 캔버스 앱 버전으로 롤백되지 않습니다. 
-   캔버스 앱 액세스(CRUD 및 보안)는 PowerApps에서 전적으로 관리되고 Common Data Service 데이터베이스는 사용되지 않습니다.
-   캔버스 앱을 호출하는 CDS AP는 차단되고 아무것도 반환하지 않습니다 
-   솔루션에서 만든 캔버스 앱을 아직 AAD 보안 그룹에 대한 공동 소유자로 공유할 수 없습니다.
-   기본 솔루션 탐색기에 캔버스 앱이 표시되지 않음 
-   기존 캔버스 앱이 솔루션을 인식하지 못함 
