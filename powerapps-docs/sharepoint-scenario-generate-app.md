...
**팁:** 이 시나리오에 대한 [다운로드 패키지](https://aka.ms/o4ia0f)에는 이 앱의 완성된 버전인 project-requests-app.msapp이 포함되어 있습니다.
...
3. 가운데 창에서 보려는 각 화면을 클릭하거나 누릅니다. 화면에는 세 가지가 있습니다.
   ...
   1. 목록에서 데이터를 찾아보고, 정렬하고, 필터링하고 추출할 수 있는 **찾아보기** 화면
   2. 항목에 대한 자세한 정보를 볼 수 있는 **세부 정보** 화면
   3. 기존 항목을 편집하거나 새 항목을 만들 수 있는 **편집/만들기** 화면
...
3. 오른쪽 창에서 필드를 업데이트하여 다음 목록이 일치하는지 확인합니다.
...
4. **BrowseGallery1**을 선택한 상태에서 **항목** 속성을 선택합니다.
   
    ![항목 속성](./media/sharepoint-scenario-generate-app/02-03-03-items.png)
5. 수식을 **SortByColumns(Filter('Project Requests', StartsWith(Title, TextSearchBox1.Text)), "Title", If(SortDescending1, Descending, Ascending))**으로 변경합니다.
...
## <a name="step-4-customize-the-apps-details-screen-and-edit-screen"></a>4단계: 앱의 세부 정보 화면 사용자 지정 및 화면 편집
