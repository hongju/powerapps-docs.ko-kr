...
PowerApps를 통해 사용자는 조직의 일상 업무에 중요한 부분인 기간 업무 애플리케이션을 빌드할 수 있습니다. 사용자가 조직을 떠나면 사용자가 만든 특정 데이터 및 리소스를 삭제할지 여부를 수동으로 검토하고 결정해야 합니다. 다른 고객 데이터는 사용자 계정이 Azure Active Directory에서 삭제될 때마다 자동으로 삭제됩니다.
