---
title: 고객 데이터에 대한 DSR(Data Subject Rights) 삭제 요청에 응답 | Microsoft Docs
description: 고객 데이터에 대한 DSR(Data Subject Rights) 삭제 요청에 응답
services: powerapps
suite: powerapps
...
---
# <a name="responding-to-delete-data-subject-rights-dsr-requests-for-customer-data-in-powerapps"></a>PowerApps에서 고객 데이터에 대한 DSR(Data Subject Rights) 삭제 요청에 응답

조직의 고객 데이터에서 개인 데이터 제거를 통한 “삭제 권한”은 GDPR(일반 데이터 보호 규정)의 주요 보호 방법입니다. 개인 데이터 제거에는 시스템 생성 로그 제거가 포함되지만 감사 로그 정보 제거는 포함되지 않습니다.
...
\*** 앱용 CDS(Common Data Service) 데이터베이스를 포함하는 환경의 경우 환경 권한(환경 작성자 및 관리자 역할에 할당된 사용자)은 해당 데이터베이스에 레코드로 저장됩니다. 앱용 CDS의 사용자에 대한 DSR에 응답하는 방법에 대한 자세한 내용은 [Executing DSRs against Common Data Service Customer Data](https://go.microsoft.com/fwlink/?linkid=872251)(Common Data Service 고객 데이터에 대해 DSR 실행)를 참조하세요.
...
캔버스 앱  | PowerApps 관리 센터 <br> PowerApps 사이트| PowerApps cmdlet
...
\** 앱용 CDS가 도입됨에 따라 환경 내에서 데이터베이스를 만드는 경우 환경 권한 및 모델 기반 앱 권한은 해당 데이터베이스의 인스턴스 내에 레코드로 저장됩니다. 앱용 CDS의 사용자에 대한 DSR에 응답하는 방법에 대한 자세한 내용은 [Executing DSRs against Common Data Service Customer Data](https://go.microsoft.com/fwlink/?linkid=872251)(Common Data Service 고객 데이터에 대해 DSR 실행)를 참조하세요.

## <a name="prerequisites"></a>필수 조건

### <a name="for-users"></a>사용자의 경우
유효한 PowerApps 라이선스가 있는 사용자는 [PowerApps 사이트](https://web.powerapps.com) 또는 [앱 작성자용 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448)을 사용하여 이 문서에 설명된 사용자 작업을 수행할 수 있습니다.

### <a name="for-administrators"></a>관리자의 경우
[PowerApps 관리 센터](https://admin.powerapps.com/), Microsoft Flow 관리 센터 또는 [PowerApps 관리자 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804)을 사용하여 이 문서에 설명된 관리 작업을 수행하려면 다음 권한이 둘 다 있는 계정이 필요합니다.

- PowerApps 요금제 2에 대한 유료 또는 평가판 라이선스. [평가판 라이선스에 등록](http://web.powerapps.com/trial)하고 30일 후에 갱신할 수 있습니다.
...
관리자는 다음 단계를 통해 [PowerApps 관리 센터](https://admin.powerapps.com/)에서 특정 사용자가 만든 환경에 대한 관리 액세스 권한을 부여할 수 있습니다.
...
관리자는 다음 단계를 통해 [PowerApps 관리 센터](https://admin.powerapps.com/)에서 특정 사용자가 만든 환경을 검토하고 삭제할 수 있습니다.
...
관리자는 [PowerApps 관리자용 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804)의 **Set-AdminEnvironmentRoleAssignment** 함수를 통해 사용자가 만든 모든 환경에 대한 액세스 권한을 자신(또는 조직 내 다른 사용자)에게 할당할 수 있습니다.
...
### <a name="delete-environments-created-by-a-user-using-powershell"></a>PowerShell을 사용하여 사용자가 만든 환경 삭제
 관리자는 [PowerApps 관리자용 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804)의 **Remove-AdminEnvironment** 함수를 통해 사용자가 만든 모든 환경을 삭제할 수 있습니다.
...
관리자는 다음 단계를 통해 [PowerApps 관리 센터](https://admin.powerapps.com/)에서 사용자의 환경 권한을 삭제할 수 있습니다.
...
4.  **환경 역할**에서 **환경 관리자** 및 **환경 작성자**를 각각 둘 다 선택한 다음 검색 창을 사용하여 사용자 이름을 검색합니다.
...
관리자는 [PowerApps 관리자용 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804)의 **Remove-AdminEnvironmentRoleAssignment** 함수를 통해 앱용 CDS 데이터베이스가 없는 모든 환경에서 사용자에 대한 모든 환경 역할 할당을 삭제할 수 있습니다.
...
관리자가 사용자의 캔버스 앱을 삭제하지 않도록 결정하면 [PowerApps 관리자 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804)의 **Set-AdminAppOwner** 함수를 통해 사용자가 소유한 앱을 다시 할당할 수 있습니다.
...
관리자는 다음 단계를 통해 [PowerApps 관리 센터](https://admin.powerapps.com/)에서 사용자가 만든 앱을 삭제할 수 있습니다.
...
### <a name="powerapps-admin-center"></a>PowerApps 관리 센터
관리자는 다음 단계를 통해 [PowerApps 관리 센터](https://admin.powerapps.com/)에서 사용자에 대한 앱 역할 할당을 삭제할 수 있습니다.
...
6.  사용자가 앱에 액세스할 수 있는 경우에는 앱의 **공유** 화면에서 해당 권한을 제거하고 **저장**을 선택합니다.
...
Microsoft Flow 서비스를 사용하는 사용자에 대한 DSR에 응답하는 방법에 대한 자세한 내용은 [Executing DSRs against Microsoft Flow customer data](https://go.microsoft.com/fwlink/?linkid=872250)(Microsoft Flow 고객 데이터에 대해 DSR 실행)를 참조하세요.
...
앱용 CDS를 사용하는 사용자에 대한 DSR에 응답하는 방법에 대한 자세한 내용은 [Executing DSRs against customer data in CDS for Apps](https://go.microsoft.com/fwlink/?linkid=872251)(앱용 CDS에서 고객 데이터에 대해 DSR 실행)를 참조하세요.
